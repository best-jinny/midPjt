...
<script type="text/javascript">
	...
	function hashReplace(hash_name) {
		...
		$('#hashtagList').append(hashReplaceData1+hash_name_insert+hashReplaceData2);
	}
	function hashCreate(hash_name) {
		...
		var hashCreateData2 = '\')" value="'+hash_name+
			'" class="button-hash"></div>';
		$('#hashtagList').append(hashCreateData1+hash_name_insert+hashCreateData2);
	}
...
<body>
<h6 class="warntext">*등록하고 싶은 해시태그를 클릭해주세요</h6>
<br>
<c:if test="${needNew }">
	<input type="button" onclick="hashCreate('${newHashtag}')" value=" ${newHashtag } " class="button-new">
</c:if>
<c:if test="${not empty listHdSearch }">
	<c:forEach var="hashtag" items="${listHdSearch }">
		<input type="button" onclick="hashReplace('${hashtag.hash_name }')" value=" ${hashtag.hash_name } " class="button-style">
	</c:forEach>
</c:if>
</body>
</html>